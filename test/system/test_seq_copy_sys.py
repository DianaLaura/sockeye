...
from test.common import check_train_translate, check_train_translate_frame_embed
...
def test_seq_copy_frame_embeddings(name, train_params, translate_params, use_prepared_data, perplexity_thresh, bleu_thresh):
    """Task: copy short sequences of digits, this time with frame embeddings"""
    with tmp_digits_timestamp_dataset(prefix="test_seq_copy_frame_embeddings",
                            train_line_count=_TRAIN_LINE_COUNT,
                            train_line_count_empty=_TRAIN_LINE_COUNT_EMPTY,
                            train_max_length=_LINE_MAX_LENGTH,
                            dev_line_count=_DEV_LINE_COUNT,
        ...
        data = check_train_translate_frame_embed(train_params=train_params,
                                     translate_params=translate_params,
                                     data=data,
                                     use_prepared_data=use_prepared_data,
                                     max_seq_len=_LINE_MAX_LENGTH,
        ...
        logger.info("test results: %s", name)
        #logger.info("perplexity=%f, bleu=%f, bleu_restrict=%f chrf=%f", perplexity, bleu, bleu_restrict, chrf)
        logger.info("perplexity=%f, bleu=%f, chrf=%f", perplexity, bleu, chrf)
        logger.info("================\n")
        assert perplexity <= perplexity_thresh
        assert bleu >= bleu_thresh
        #assert bleu_restrict >= bleu_thresh